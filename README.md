...
> Windows 进程守护与定时管理工具 —— 监控指定程序，异常退出后自动重启，并提供定时重启、定时运行、启动延时、失败重试、邮件提醒、自我守护等完整保障机制。
...
- 快捷方式路径：输入 `.lnk` 文件路径，自动解析快捷方式目标程序
...
将 SuperGuardian 主窗口置顶到所有窗口之上，再次点击可取消置顶。适合需要同时操作其他程序时保持守护列表可见。
...
- 可通过 **配置 → 允许重复添加的程序** 管理白名单，添加后白名单内的程序可多次加入列表
...
| 菜单项 | 说明 |
|--------|------|
| 守护日志 | 打开 `Logs/watchdog.log`，查看守护触发记录 |
| 定时重启日志 | 打开 `Logs/scheduled_restart.log` |
| 定时运行日志 | 打开 `Logs/scheduled_run.log` |
...
| 定时重启规则 | 配置定时重启规则（与定时运行互斥） |
| 设置启动延时 | 设置守护重启前的等待秒数 |
| 定时运行规则 | 配置定时运行规则（与守护/定时重启互斥） |
| 重试设置 | 配置失败重试的间隔、次数、时长 |
| 邮件提醒设置 | 配置该程序的邮件提醒事件 |
| 复制定时重启规则 | 复制当前程序的定时重启规则 |
| 复制定时运行规则 | 复制当前程序的定时运行规则 |
| 打开文件所在的位置 | 在资源管理器中定位程序文件 |
| 移除项 | 从列表中移除程序（守护/定时任务激活时禁用） |

> 定时运行开启时，定时重启规则和启动延时菜单项会被禁用；守护或定时重启开启时，定时运行规则菜单项会被禁用。
...
所有配置保存在程序同级目录的 `Config/` 文件夹下：

| 文件 | 内容 |
|------|------|
| `Config/settings.json` | 全部程序配置、守护状态、定时规则、邮件设置等 |

### 日志文件

运行日志保存在程序同级目录的 `Logs/` 文件夹下：

| 文件 | 内容 |
|------|------|
| `Logs/watchdog.log` | 守护触发重启记录、看门狗自我守护记录 |
| `Logs/scheduled_restart.log` | 定时重启记录 |
| `Logs/scheduled_run.log` | 定时运行记录 |
...
# 4. 打包发布
powershell -File package-release.ps1 -Version "1.0"
```
...
| `package-release.ps1` | 仅打包（不编译，适用于已编译的 Release 版本） |
