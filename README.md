...
| 🌐 全局功能开关 | 一键全局禁用/启用所有守护、所有定时重启或所有定时运行，方便临时暂停全部任务 |
| 🧰 工具栏快捷按钮 | 工具栏图标按钮可快速切换常用选项（选项组：自我守护/开机自启/启动最小化；功能组：全局守护/全局定时重启/全局定时运行/邮件提醒），激活状态高亮显示 |
...
- Windows 关机、重启或注销时会停止看门狗，不会把系统强制结束误报为异常退出，也不会为该场景生成 `.dmp`
- 主程序发生未处理异常时，会在 `SuperGuardian.exe` 所在目录生成 `crash_*.dmp`，看门狗重启后会显示转储路径
- 可通过 **帮助 → 测试自我守护** 触发模拟崩溃，同时验证崩溃转储和自动恢复
- 任务管理器“结束进程/结束进程树”属于系统硬终止，不会执行进程内崩溃处理器；结束主进程仍可验证自动拉起，但不会产生有效崩溃 Dump
- 看门狗事件记录在软件运行日志中
...
- 通过 **帮助 → 测试程序是否允许重复添加** 可验证某个程序是否被白名单覆盖
...
SuperGuardian 的菜单栏包含以下菜单（查看 / 选项 / 功能 / 配置 / 操作 / 帮助），工具栏右侧有置顶（📌）和主题切换按钮：

### 工具栏

工具栏位于菜单栏下方，提供常用功能的图标快捷按钮，激活时显示高亮背景：

**选项组**（对应"选项"菜单）：自我守护 · 开机自启 · 启动时最小化到托盘

**功能组**（对应"功能"菜单）：全局守护 · 全局定时重启 · 全局定时运行 · 邮件提醒
...
| 菜单项 | 说明 |
|--------|------|
| 全局守护 | 全局启用/禁用所有守护（可勾选）；关闭时暂停所有守护，重新开启后自动恢复 |
| 全局定时重启 | 全局启用/禁用所有定时重启（可勾选） |
| 全局定时运行 | 全局启用/禁用所有定时运行（可勾选） |
| 邮件提醒 | 全局邮件提醒开关（可勾选），与工具栏及托盘菜单同步 |
